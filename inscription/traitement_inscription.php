...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  /*  if(idCandidatExiste($_SESSION['id_user'],$connexion,$_SESSION['annee'])){
    ...
    if( !isSpecialiteInEtablissement($connexion,$specialite,$etablissement)){
        header("location: index?erreur=la specialité choisie ne correspond pas à l'etablissement.");
        exit;
    }
    ...
    $sql = "INSERT INTO candidat (code,nom, prenom, email, date_nais, lieu_nais, sexe, bac,moyenneBac, anneebac, mention, cycle, etab, specialite, annee,tel,statut,img,utilisateur,date_cand,statut_paiement,statut_paiement_concours,univ,etat) VALUES (
                            '$code','$nom', '$prenom', '$email', '$dateNaissance', '$lieuNaissance', '$sexe', '$bac',$moybac, '$anneeBac', '$mention', '$cycle', '$etablissement', '$specialite', '$annee','$tel','en cours','$nouveauChemin',".$_SESSION['id_user'].",'".date("Y-m-d")."',0,0,'UDSN',1)";
    ...
    if ($connexion->query($sql) === TRUE) {
        ...
        header("location: index?sucess='L'opération a était effectué avec succès. Code d'enregistrement = $code'&candidat=$code");
    } else {
