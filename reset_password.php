...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($user_type === 'enseignant') {
            handleEnseignantReset($conn);
        } elseif ($user_type === 'etudiant') {
            handleEtudiantReset($conn);
        } else {
            header("Location: ../forgot_password.php?erreur=" . urlencode("Type de compte invalide"));
            exit();
        }
    } catch (Exception $e) {
        header("Location: ../forgot_password.php?erreur=" . urlencode("Erreur système: " . $e->getMessage()));
        exit();
    }
} else {
...
function handleEnseignantReset($conn) {
...
function handleEtudiantReset($conn) {
    // Récupération des données
