...
if( $_SESSION['id'] == session_id() and  $_SESSION['role']=="scolarité"){
  ...
  if(isset($_GET['sup'])){
        ...
        // Delete the demande
        $sql_delete = "DELETE FROM demande WHERE code=? AND etab=? AND statut=0";
        $stmt = $connexion->prepare($sql_delete);
        $stmt->bind_param("ss", $code, $_SESSION['etablissement']);
        
        if($stmt->execute()){
            // Redirect with success message
            header("location: gestion?sucess=Demande supprimée avec succès");
            exit();
        } else {
            // Redirect with error message
            header("location: gestion?erreur=Erreur lors de la suppression");
            exit();
        }
    }
