...
if(isset($_GET['semestre']) and isset($_GET['specialite']) and isset($_GET['annee']) and isset($_GET['examen'])  and isset($_GET['classe']))
{
...
<table id="example3" class="display" >
    ...
    <?php 
          ...
          $sql1="select inscription.id as id from inscription join classe on inscription.classe=classe.libelle join specialite on classe.specialite=specialite.libelle join parcours on specialite.parcours=parcours.libelle where classe.niveau='$niveau' and parcours.libelle='$parcours' and classe.libelle='$classe'";
...
<th class="small-column text-dark"> <?php 
    ...
    while($data_count = $result_ue_count->fetch_object()){
        ...
        while($ecue_count = $result_ecue_count->fetch_object()){
            $total_ecues++;
            $cc = getEtudiantCC($etudiant->id,$connexion,$_SESSION["etablissement"],$semestre,$ecue_count->libelle,$annee);
            
            if($examen == "ordinaire"){
                $ex = getEtudiantEXT($etudiant->id,$connexion,$_SESSION["etablissement"],$semestre,$ecue_count->libelle,$annee);
            } else {
                $ex = getEtudiantRattrapage($etudiant->id,$connexion,$_SESSION["etablissement"],$semestre,$ecue_count->libelle,$annee);
            }
    ...
    // Only calculate average if at least 90% of ECUEs have data
