...
if( $moyenne < 10 and $decision == "Admis"){
    header("location: index?erreur='Cette décision est incorrecte car l'etudiant n'a pas atteint la moyenne.'");
    exit;
    
}

if( $moyenne >= 10 and $decision == "Ajourné"){
    header("location: index?erreur='Cette décision est incorrecte car l'etudiant a atteint la moyenne. $moyenne'");
    exit;
    ...
    if($tester->num_rows > 0){
        ...
        if($connexion->query("update recap set decision='$decision',examen='$examen' where id=$id")){
            ...
            header("location: index?sucess=Modification effectuée avec succes $moyenne");
        ...
        if($connexion->query($sql)){
         ...
         header("location: index?sucess=Modification effectuée avec succes ");
